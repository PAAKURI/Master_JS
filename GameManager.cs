...
public partial class GameManager : Node2D
{
    ...
    private void ApplyCharacterLooks()
    {
        ...
        if (_session.IsClient)
        {
            var localPlayer = _session.LocalPlayerSlot == 1 ? _playerOne : _playerTwo;
            var remotePlayer = _session.LocalPlayerSlot == 1 ? _playerTwo : _playerOne;
            localPlayer.ApplyCustomization(customization.LocalLook);
            remotePlayer.ApplyCustomization(customization.RemoteLook);
            return;
        }
